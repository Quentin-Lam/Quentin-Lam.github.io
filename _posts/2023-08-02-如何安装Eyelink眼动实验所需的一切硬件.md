...
def run_a_trail(subjInfo, trial_info, rec):
    ...
    if rec == 1:
    	...
    	core.wait(2)
        img_e = draw_pic(os.path.join(d_dir_learn, stim_e), pos = e_loc)
        img_n = draw_pic(os.path.join(d_dir_learn, stim_n), pos = n_loc)
    core.wait(2.5)
    if rec == 1:
        tk.sendMessage("trial ok1")
        t_tar_onset = core.getTime()
    draw_dot([0, 1, 0], dot_loc)
    event.waitKeys(200000, key_list, timeStamped=True)
...
d_dir_learn = proj_root + 'exp_stim/prac'
stim_p1 = ['1.bmp']
stim_p2 = ['2.bmp']
