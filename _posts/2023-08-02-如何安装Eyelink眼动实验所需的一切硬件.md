...
def generate_trial_list(cue_pos, LoEp, dot_pos, stim_e, stim_n):
    trial_list = []
    for i in range(len(stim_e)):
        list1 = [cue_pos, LoEp, dot_pos, stim_e[i], stim_n[i]]
...
for i in cue_pos:
    for j in LoEp:
        for k in dot_pos:
            trial_list_condition = generate_trial_list(i, j, k, file_name_Exp, file_name_N)
            trial_list_rec.extend(trial_list_condition)

# print(trial_list_rec[0]) # [1, 1, 1, 'FF17.bmp', 'NF15.bmp']
```
...
def run_a_trail(subjInfo, trial_info, rec):
    ...
    if rec == 1: # if it's the experiment phase, we should activate eye-tracker
    	...
    	cue_pos, LoEp, dot_pos, stim_e, stim_n = pars
    else: # in the learning phase, we need less info
        cue_pos, LoEp, dot_pos, right_resp = pars
...
learnlist = [[1, 1, 1, 'z'], [1, 1, 2,'z'], [1, 1, 3,'z'], [2, 2, 1, 'slash'], [2, 2, 2, 'slash'], [2, 2, 3, 'slash']]
