...
def driftCheck4(fixDotX, fixDotY, fixDur=500, acceptableDev=1*deg2pix)
    '''This is an optional function. It's to make sure that the trial only starts when the eyes gazed at the cue dot for long enough time (>= fixDur). Use draw_dot if you don't need this. ''' 
    ...
    fix = visual.GratingStim(surf, tex='None',mask='circle', size=deg2pix,  pos=(fixDotX,fixDotY))
	fix.draw()
	surf.flip()
	triggered = False
	fixationStartTime = -1
    while not triggered:
		...
		if dt > 0:
			...
			if dt == pylink.FIXUPDATE:
				...
				#fix111.draw()
				surf.flip()
				gazeError = hypot((gazeX - pw / 2) - fixDotX, ( ph / 2 - gazeY) - fixDotY)
...
def run_a_trail(subjInfo, trial_info, rec):
    ...
    img_e = draw_pic(os.path.join(d_dir_all, stim_e), pos = pos_e)
    img_n = draw_pic(os.path.join(d_dir_all, stim_n), pos = pos_n)
    
    if rec == 1:
        pylink.msecDelay(50)
        driftCheck4(fix_loc[0], fix_loc[1])
```
...
stim_p1 = ['1.bmp']
stim_p2 = ['2.bmp']
learnlist = generate_trial_list(cue_pos, LoEp, dot_pos, stim_p1, stim_p2)
